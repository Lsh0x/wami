...
use crate::store::{StsStore, SsoAdminStore};
use crate::sts::{StsSession, CallerIdentity};
use crate::sso_admin::{PermissionSet, AccountAssignment, SsoInstance, Application, TrustedTokenIssuer};
...
impl StsStore for InMemoryStsStore {
    ...
    async fn create_identity(&mut self, identity: CallerIdentity) -> Result<CallerIdentity> {
        self.identities.insert(identity.arn.clone(), identity.clone());
        Ok(identity)
    }
...
impl SsoAdminStore for InMemorySsoAdminStore {
    async fn create_permission_set(&mut self, permission_set: PermissionSet) -> Result<PermissionSet> {
        self.permission_sets.insert(permission_set.permission_set_arn.clone(), permission_set.clone());
        Ok(permission_set)
    }
    ...
    async fn update_permission_set(&mut self, permission_set: PermissionSet) -> Result<PermissionSet> {
        self.permission_sets.insert(permission_set.permission_set_arn.clone(), permission_set.clone());
        Ok(permission_set)
    }
    ...
    async fn create_account_assignment(&mut self, assignment: AccountAssignment) -> Result<AccountAssignment> {
        let assignment_id = format!("{}-{}-{}", assignment.account_id, assignment.permission_set_arn, assignment.principal_id);
        self.account_assignments.insert(assignment_id, assignment.clone());
        Ok(assignment)
    }

    async fn get_account_assignment(&self, assignment_id: &str) -> Result<Option<AccountAssignment>> {
    ...
    async fn list_account_assignments(&self, account_id: &str, permission_set_arn: &str) -> Result<Vec<AccountAssignment>> {
        let assignments: Vec<AccountAssignment> = self.account_assignments
            .values()
    ...
    async fn create_instance(&mut self, instance: SsoInstance) -> Result<SsoInstance> {
        self.instances.insert(instance.instance_arn.clone(), instance.clone());
        Ok(instance)
    }
    ...
    async fn create_application(&mut self, application: Application) -> Result<Application> {
        self.applications.insert(application.application_arn.clone(), application.clone());
        Ok(application)
    }
    ...
    async fn create_trusted_token_issuer(&mut self, issuer: TrustedTokenIssuer) -> Result<TrustedTokenIssuer> {
        self.trusted_token_issuers.insert(issuer.trusted_token_issuer_arn.clone(), issuer.clone());
        Ok(issuer)
    }

    async fn get_trusted_token_issuer(&self, issuer_arn: &str) -> Result<Option<TrustedTokenIssuer>> {
    ...
    async fn list_trusted_token_issuers(&self, instance_arn: &str) -> Result<Vec<TrustedTokenIssuer>> {
