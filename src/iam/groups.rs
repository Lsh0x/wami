...
impl<S: Store> IamClient<S> {
    ...
    pub async fn update_group(&mut self, request: UpdateGroupRequest) -> Result<AmiResponse<Group>> {
        if let Some(mut group) = self.groups.remove(&request.group_name) {
            ...
            self.groups.insert(updated_group.group_name.clone(), updated_group.clone());
    ...
    /// List all groups
    pub async fn list_groups(&self, request: Option<ListGroupsRequest>) -> Result<AmiResponse<ListGroupsResponse>> {
    ...
    /// Add user to group
    pub async fn add_user_to_group(&mut self, group_name: String, user_name: String) -> Result<AmiResponse<()>> {
    ...
    pub async fn remove_user_from_group(&mut self, group_name: String, user_name: String) -> Result<AmiResponse<()>> {
        // Check if group exists
        if !self.groups.contains_key(&group_name) {
            return Err(crate::error::AmiError::ResourceNotFound { 
                resource: format!("Group: {}", group_name) 
